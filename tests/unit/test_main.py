...
class TestSummarizeCommitData:
    ...
    def test_summarize_commit_data_with_commits(self, sample_commit_data):
        ...
        assert summary.start_date == datetime(2025, 9, 15, 10, 30, 0, tzinfo=UTC)
        assert summary.end_date == datetime(2025, 9, 16, tzinfo=UTC)
    ...
    def test_summarize_commit_data_empty_list(self):
        ...
        with patch("weekly_report_prompt.main.datetime") as mock_datetime:
            mock_now = datetime(2025, 9, 17, 12, 0, 0, tzinfo=UTC)
...
class TestMain:
    ...
    def test_main_function_success_flow(
        ...
        mock_file_mgr = Mock()
        mock_file_mgr.get_last_report_date.return_value = datetime(2025, 9, 10, tzinfo=UTC)
        ...
        mock_file_mgr_class.assert_called_once_with(config_loader=mock_config, build_dir=None)
    ...
    def test_main_function_no_previous_report(
        self, mock_git_collector_class, mock_file_mgr_class, mock_config_class, mock_console_class, sample_commit_data
    ):
    ...
    def test_main_function_no_commits_found(
        self, mock_git_collector_class, mock_file_mgr_class, mock_config_class, mock_console_class
    ):
    ...
    def test_main_function_large_token_count_warning(
        ...
        warning_calls = [
            call for call in mock_console.print.call_args_list if len(call[0]) > 0 and "very large" in str(call[0][0])
        ]
    ...
    def test_main_function_multiple_repositories(
        ...
        mock_config = Mock()
        mock_config.get_repositories.return_value = ["/path/to/repo1", "/path/to/repo2", "/path/to/repo3"]
        mock_config.get_large_prompt_tokens.return_value = 150_000
        mock_config_class.return_value = mock_config
